...
class PolyMetisServer:
    ...
    def init_robot(self):
        self._robot = polymetis.RobotInterface(
            ip_address=self.robot_ip, enforce_version=False
        )
        self._gripper = polymetis.GripperInterface(ip_address=self.robot_ip)
        if hasattr(self._gripper, "metadata") and hasattr(
            self._gripper.metadata, "max_width"
        ):
