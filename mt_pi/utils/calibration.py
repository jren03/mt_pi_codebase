"""
Script credit to https://github.com/priyasundaresan/sphinx/blob/main/interactive_scripts/vision_utils/calib_utils.py
"""
...
def detect_calibration_marker(image):
    ...
    # Draw rectangles around detected red blocks
...
def transform_point(pt, transform_matrix):
    npt = np.ones(4)
    npt[:3] = pt
    return np.dot(transform_matrix, npt)
...
if __name__ == "__main__":
    loaded_robot_pts = np.load("../calib/robot_pts.npy", allow_pickle=True)
    loaded_cam_pts = np.load("../calib/cam_pts.npy", allow_pickle=True).item()
    loaded_cam_pts = np.array(loaded_cam_pts[list(loaded_cam_pts.keys())[0]])
    print(loaded_robot_pts.shape, loaded_cam_pts.shape)
    solver = Solver()
    solver.solve_transforms(loaded_robot_pts, loaded_cam_pts)
